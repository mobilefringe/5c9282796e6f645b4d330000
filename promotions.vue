				...
				<!--<paginate name="promos" v-if="promos" :list="promos" class="paginate-list margin-60" :per="4">-->
					<div class="row event_container" v-if="showMore > index" v-for="(promo,index) in promos">
						...
						<div class="col-sm-6 col-md-9 event_dets_container">
							...
							<p class="event_dates" v-if="isMultiDay(promo)">{{promo.start_date | moment("MMM D", timezone)}} - {{promo.end_date | moment("MMM D", timezone)}} </p>
							<p class="event_dates" v-else>{{ promo.start_date | moment("MMM D", timezone) }}</p>
...
<script>
    define(["Vue", "vuex", "moment", "moment-timezone", "vue-moment", "vue-meta", "vue-lazy-load", "vue-paginate", "vue!sponsorship"], function(Vue, Vuex, moment, tz, VueMoment, Meta, VueLazyload, VuePaginate, sponsorship) {
        Vue.use(Meta);
        Vue.use(VueLazyload);
        Vue.use(VuePaginate);
